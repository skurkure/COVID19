...
#population
N <- covidvax$population[2] #total population on Feb 1, 2021
...
#set arbitrary initial conditions-try Feb 2021
#this is the Delta wave
N <- 22632532
flat <- covidvax %>%
  filter(Submission_Date >="2021-03-01" & Submission_Date <= "2022-04-30")
flat_vax_death <- flat %>%
  pull(cum_vax_deaths)
flat$date
Day <- 1:(length(flat))
which(covidvax$Submission_Date == "2021-03-01") #335
flat_data <- covidvax %>%
  filter(Submission_Date >= "2021-03-01" & Submission_Date < "2021-04-30") %>%
  pull(cum_cases)
flat_data
...
t <- 151:307
#vax data
flat_vax_data <- covidvax %>%
  filter(Submission_Date >= "2021-03-01" & Submission_Date <= "2022-04-30") %>%
  pull(cum_vax)
#nonvax data now
flat_unvax_data <- covidvax %>%
  filter(Submission_Date >= "2021-03-01" & Submission_Date <= "2022-04-30") %>%
  pull(cum_unvax)
...
RSS_new <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_vax_data - fit)^2)
}

init_new <- c(S = flat$population_vaccinated[1]-flat$cum_vax[1], I = flat$I_vax_active[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
Opt_new <- optim(c(0.5), RSS_new, method = "Brent", lower = c(0), upper = c(1))
...
RSS_new2 <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_unvax_data - fit)^2)
}

init_new2 <- c(S = flat$population_unvaccinated[1]-flat$cum_unvax[1], I = flat$I_unvax_active[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
...
RSS3 <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_vax_death - fit)^2)
}

init3 <- c(I = flat$I_vax_active[1], D = flat$cum_vax_deaths[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
...
RSS4 <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_unvax_death - fit)^2)
}

init4 <- c(I = flat$I_unvax_active[1], D = flat$cum_unvax_deaths[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
...
RSS5 <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_vax_recovered - fit)^2)
}

init5 <- c(I = flat$I_vax_active[1], R = flat$R_vax[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
...
RSS6 <- function(parameters) {
  ...
  fit <- out[ , 3]
  sum((flat_unvax_recovered - fit)^2)
}

init6 <- c(I = flat$I_unvax_active[1], R = flat$R_unvax[1])
#Nelder-Mead”, “BFGS”, “CG”, “L-BFGS-B”, “SANN”, “Brent"
#optimization
Opt6 <- optim(c(0.5), RSS6, method = "Brent", lower = c(0), upper = c(1))
#check for convergence
Opt6$message
...
#I think flat wave is around the end of this dataset?
