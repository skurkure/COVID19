...
init <- c(Sv = 33922997, Su = 4877123, Iv = 3, Iu = 523, R = 295000, D = 0)
parameters <- c(N = 34218000, beta1 = 0.619, beta2 = 0.589, kappa1 = 0.0183, kappa2 = 0.0118, epsilon = 0.0003, sigma = 0.05, rho1 = 0.00019/0.0183, rho2 = 0.00019/0.0118, alpha = 0.8419, lambda1=0.1765/(1-0.0183),lambda2=0.01/(1-0.0118))
times      <- seq(0, 500, by = 1)
#solve the system of ODE's
out <- ode(y = init, times = times, func = pmodel, parms = parameters)
#convert to dataframe
...
pmodel_graph_noS <- out %>%
  ...
  geom_line(aes(y= R, color = "Recovered")) +
  geom_line(aes(y= D, color = "Dead")) +
  geom_line(aes(y=head(cumulative_fully_vaccinated,501), color = "Real")) +
  scale_y_continuous(labels = scales::comma) +
  scale_color_manual(name = "",
                     breaks = c("Infectious-vaccinated", "Infectious-unvaccinated", "Recovered", "Dead", "Real"),
                     values = c("black", "brown", "green", "blue", "pink")) + theme_bw()
pmodel_graph_noS
