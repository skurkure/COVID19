...
#population 
N <- covidvax$population[30] #total population on March 1
...
flat$date
Day <- 1:(length(flat))
which(covidvax$Submission_Date == "2021-03-01") #151
...
times_61 <- seq(1, 61, by = 1)
#solve the system of ODE's
out_2 <- ode(y = init, times = times_61, func = pmodel, parms = parameters)
#convert to dataframe
out_2 <- as.data.frame(out_2)
pmodel_graph_flat_with_data <- ggplot(out_2, aes(x=times_61)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
  geom_line(aes(y= Sv, color = "Susceptible-vaccinated")) +
...
pmodel_graph_flat_with_data_no_S <- ggplot(out_2, aes(x=times_61)) +
  labs(y="Individuals",
        x="Time",
        title="Modified SIRD Vaccination Model, Simulated") +
  geom_line(aes(y= Iv, color = "Infectious-vaccinated")) +
...
head(out_3, 100)
#visualize
pmodel_graph_flat_epsilon_3 <- out_3 %>%
  ggplot(aes(x = times)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
...
head(out_4, 100)
#visualize
pmodel_graph_flat_epsilon_4 <- out_4 %>%
  ggplot(aes(x = times)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
...
head(out_4, 100)
#visualize
pmodel_graph_flat_epsilon_10x <- out_4 %>%
  ggplot(aes(x = times)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
...
head(out_5, 100)
#visualize
pmodel_graph_flat_epsilon_5 <- out_5 %>%
  ggplot(aes(x = times)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
...
head(out_6, 100)
#visualize
pmodel_graph_flat_epsilon_6 <- out_6 %>%
  ggplot(aes(x = times)) +
  labs(y="Individuals",
       x="Time",
       title="Modified SIRD Vaccination Model, Simulated") +
